...
///
/// Every layer name here is nested under `tauler.*`, not top-level — the docs site also
/// declares top-level `theme`/`utilities` layers (its own Tailwind build), and CSS layers
/// merge by name across the whole page. A bare `@layer utilities` here would fold into the
/// site's *own* `utilities` layer instead of getting a fresh slot, which silently moves
/// `tauler-reset` from "declared below utilities" (the invariant this comment used to
/// assert) to "the last new name in the document" — i.e. highest priority, backwards from
/// what `all: revert` needs. Starlight avoids the same trap with `starlight.*`; same fix.
fn stylesheet_input(font_url: &str) -> String {
    format!(
        r#"/* Generated by tauler-docgen. Do not edit by hand. */
@layer tauler.reset, tauler.theme, tauler.utilities;
...
@import "tailwindcss/theme.css" layer(tauler.theme);
@import "tailwindcss/utilities.css" layer(tauler.utilities);
@source "./classes.txt";
...
mod tests {
    ...
    fn the_reset_layer_is_declared_before_tailwinds() {
        ...
        let order = css
            .find("@layer tauler.reset, tauler.theme")
            .expect("layer order");
