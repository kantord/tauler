...
mod table_header {
    ...
    fn tw_contains_border_border() {
        let tw = eval_table("<TableHeader />")["tw"].as_str().unwrap().to_string();
...
mod table_head {
    ...
    fn tw_contains_flex_1() {
        let tw = eval_table("<TableHead />")["tw"].as_str().unwrap().to_string();
    ...
    fn tw_contains_text_muted_foreground() {
        let tw = eval_table("<TableHead />")["tw"].as_str().unwrap().to_string();
        assert!(tw.contains("text-muted-foreground"), "text-muted-foreground missing: {tw}");
    }
...
mod table_cell {
    ...
    #[test]
    fn tw_contains_flex_1() {
        let tw = eval_table("<TableCell />")["tw"].as_str().unwrap().to_string();
        assert!(tw.contains("flex-1"), "flex-1 missing: {tw}");
    }
    ...
    fn tw_contains_text_foreground() {
        let tw = eval_table("<TableCell />")["tw"].as_str().unwrap().to_string();
        assert!(tw.contains("text-foreground"), "text-foreground missing: {tw}");
    }
