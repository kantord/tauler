  ...
  if (kind === 'flow') {
    ...
    for (let gy = 0; gy < h + g; gy += g)
      for (let gx = 0; gx < w + g; gx += g) {
        ctx.lineWidth = 1.3
        ctx.strokeStyle = R() < 0.18 ? W(0.09) : A(0.1 + R() * 0.12)
        ctx.beginPath()
        if (R() < 0.5) {
          ctx.arc(gx, gy, r, 0, Math.PI / 2)
          ctx.moveTo(gx + g - r, gy + g)
          ctx.arc(gx + g, gy + g, r, Math.PI, Math.PI * 1.5)
        } else {
          ctx.arc(gx + g, gy, r, Math.PI / 2, Math.PI)
          ctx.moveTo(gx + r, gy + g)
          ctx.arc(gx, gy + g, r, Math.PI * 1.5, Math.PI * 2)
        }
        ctx.stroke()
        if (R() < 0.045) {
          ctx.fillStyle = A(0.1)
          ctx.fillRect(gx + 3, gy + 3, g - 6, g - 6)
        }
      }
  } else if (kind === 'moire') {
    const cs = [
    ...
    for (let li = 0; li < 84; li++) {
      ...
      ctx.strokeStyle =
        li % 7 === 0 ? W(0.07) : A(0.07 + 0.06 * Math.abs(Math.sin(li * 0.19)))
      ctx.lineWidth = li % 7 === 0 ? 1.4 : 0.9
      ctx.stroke()
    }
  } else if (kind === 'quasi') {
    const step = 13,
      ks = [0, 1, 2, 3, 4].map((k) => (k * Math.PI) / 5)
    for (let y = step; y < h; y += step)
      for (let x = step; x < w; x += step) {
        let v = 0
        for (const a2 of ks)
          v += Math.cos((x * Math.cos(a2) + y * Math.sin(a2)) * 0.055)
        const t = (v + 5) / 10,
          rad = 0.4 + t * 3.4
        if (rad < 0.7) continue
        ctx.beginPath()
