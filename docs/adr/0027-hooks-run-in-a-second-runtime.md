...
There are two QuickJS runtimes. The render runtime evaluates the layout file on every Tick
and is pure: it produces a tree, and the Items in that tree cross to the other side as plain
JSON — a Unit name and the Item's props, nothing else. The reconciler runtime lives on its
own thread, loads the same layout file, holds the real `unit()` objects, and calls the real
hooks.
...
([0026](0026-reconcilers-are-esto-units.md)).
