...
function Reading({ label, value, tone }) {
  return (
    <div class="flex flex-col justify-center">
      <span class="text-[8px] text-muted-foreground" style={{ letterSpacing: "1.6px" }}>
        {label}
      </span>
      <span class="text-[14px]" style={{ color: tone ?? "#F1E7E2" }}>
        {value}
      </span>
    </div>
  );
...
function Divider() {
  return <div class="w-[1px] h-[26px] bg-border" />;
}
...
function RampBar({ min, max }) {
  ...
  return (
    <div class="flex flex-col justify-center gap-[3px]">
      ...
      <div class="flex flex-row justify-between w-[260px]">
        <span class="text-[8px] text-muted-foreground">{min}</span>
        <span class="text-[8px] text-muted-foreground">{max}</span>
      </div>
    </div>
  );
}
...
function ScaleStop({ label, colour }) {
  return (
    <div class="flex flex-row items-center gap-[5px] w-full">
      <div style={{ width: "9px", height: "1px", backgroundColor: colour }} />
      <span class="text-[8px] text-muted-foreground">{label}</span>
    </div>
  );
}
...
function Scale({ scene }) {
  ...
  return (
    <div
      class="flex flex-row h-full w-full pt-[16px] pb-[16px] pl-[10px] pr-[8px] bg-card"
    ...
    >
      ...
      />
      <div class="flex flex-col justify-between grow pl-[6px]">
        {stops.map((s, i) => (
...
function Measure({ geom }) {
  ...
  return (
    <panel id="measure" x={geom.x} y={geom.y} width={width} height={26} above={true}>
      <div
        class="flex flex-row items-center h-full w-full pl-[8px] pr-[10px] gap-[7px]"
        style={{ backgroundColor: "#F4D660" }}
      >
        <div
          style={{ width: "8px", height: "8px", backgroundColor: "#120F1C" }}
        />
        <span class="text-[11px]" style={{ color: "#120F1C" }}>
          {geom.label}
        </span>
      </div>
    </panel>
  ...
  return (
    <root>
      <wallpaper id="desktop">
        <div class="flex flex-col w-full h-full" style={{ ...FIELD, position: "relative" }}>
          <div
            style={{
              position: "absolute",
              bottom: "34px",
          ...
          >
            <span class="text-[13px]" style={{ color: "rgba(241,231,226,0.30)", letterSpacing: "5px" }}>
              FIELD 01 · CONTINUOUS
            </span>
            <span class="text-[10px]" style={{ color: "rgba(241,231,226,0.20)", letterSpacing: "2px" }}>
      ...
      <I3Layout module={TAULER_I3}>
        <Panel id="readout" anchor="top" size={58}>
          <div style={{ position: "relative", width: "100%", height: "100%" }}>
            ...
            />
            <div
              class="flex flex-row items-center h-full w-full pl-[16px] pr-[16px] gap-[16px] bg-card"
              style={{ position: "relative", borderBottom: "1px solid rgba(244,214,96,0.18)" }}
            >
              <div class="flex flex-row items-center gap-[8px]">
                <div style={{ width: "10px", height: "10px", backgroundColor: "#F4D660" }} />
                <span class="text-[13px] text-primary" style={{ letterSpacing: "3px" }}>
                  IR
                </span>
              </div>
