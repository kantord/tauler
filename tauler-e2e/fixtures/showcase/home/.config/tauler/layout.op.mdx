---
# Rosé Pine Moon, from the theme file beside this one: tauler's shipped palette
# is shadcn's neutral ramp, a sensible default and a poor screenshot.
theme:
  mode: dark
  file: ~/.config/tauler/theme.yaml

# The family name fontconfig actually reports, not the one on the tin: the file
# is InterVariable.ttf and `fc-list Inter` matches nothing. `extra` registers
# JetBrains Mono under its own name so theme.yaml's `fonts.readout` role can
# reach it — the clock in the bar renders in it.
fonts:
...
//
// 12 is also i3's inner gap (see home/.config/i3/config), so the space around
// the bar and the space between windows are the same distance. That is most of
// what makes the arrangement look deliberate rather than assembled.
//
// Every path in this file is one a reader has on their own machine: the
// workspace module where `cargo install tauler-i3` puts it, the status script
// and the wallpaper under $HOME. The fixture's home/ tree is copied over the
// container's $HOME, the way a dotfiles repo would be.
...
const TAULER_I3 = "~/.cargo/bin/tauler-i3";
const STATUS = "~/.local/bin/showcase-status";
  ...
  return (
    <root>
      ...
      <wallpaper id="desktop">
        <img
          src="~/.config/tauler/wallpaper.png"
